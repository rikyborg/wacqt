...
static int init_para(SimPara* para) {
    sunrealtype r0 = para->R0;  // ohm
    sunrealtype l0 = para->L0;  // H
    sunrealtype cG = para->Cg;  // F
    sunrealtype l1 = para->L1;  // H
    sunrealtype c1 = para->C1;  // F
    sunrealtype r1 = para->R1;  // ohm
    sunrealtype r2 = para->R2;  // ohm
    sunrealtype mG = para->Mg;  // H
    ...
    para->a[2][0] = (l1 + mG) / (c1*l1);
    para->a[2][1] = -1. / (c1*l1);
    para->a[2][2] = -1. / (c1*r1);
    para->a[2][3] = -1. / (c1*r2);

    para->a[3][0] = 1/cG + (l1 + mG) / (c1*l1);
    para->a[3][1] = -1. / (c1*l1);
    para->a[3][2] = -1. / (c1*r1);
    para->a[3][3] = -(c1 + cG) / (c1*cG*r2);
