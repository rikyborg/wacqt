...
print(w_c / _wc)
print(chi / _chi)
print(kappa_g / _kappa)
print(kappa_e / _kappa)
...
fc, df = para_g.tune(_fc, _df, priority='f', rounding=2)
para_g.set_df(df)
para_e.set_df(df)
...
if method == "sine" and not double:
    idx_check = np.argmin(np.abs(t_envelope - 2 * t[idx.stop]))
else:
