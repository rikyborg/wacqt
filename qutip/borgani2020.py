...
# _drive_q = np.zeros_like(tlist, dtype=np.complex128)
# for ii in range(-N // 2, 3 * N // 2):
#     if ii % 2 == 1:
#         # only odd parity
#         if METHOD == "square":
#             sign = (-1)**(ii // 2)
#         else:
#             sign = 1
#         _wd_q = (wq - wrot2) + chi + ii * 2 * chi
#         _drive_q += sign * 0.5 * amp_q * np.exp(+1j * (_wd_q * tlist + pd_q))
_drive_q = np.zeros_like(tlist, dtype=np.complex128)
_wd_q = (wq - wrot2) + chi + 0 * 2 * chi
_drive_q += 0.5 * amp_q * np.exp(+1j * (_wd_q * tlist + pd_q))

mask_q = np.zeros_like(tlist)
# 1st conditional rotation
if METHOD == "triangle":
    len_q = 2 * ns
...
end_q = start_q + len_q
# # # 2nd conditional rotation
# wait_q = wait_r // 2
# start_q += len_q + wait_q
# if METHOD == "triangle":
#     len_q = 2 * ns
#     mask_q[start_q:start_q + len_q] = triang(2 * ns)
# elif METHOD == "square":
...
# else:
#     raise NotImplementedError
# mask_q[start_q:start_q + len_q] *= -1
# end_q = start_q + len_q
...
sz_list = qt.expect(sigmaz, res.states)
sx_list = qt.expect(sigmax, res.states)
sy_list = qt.expect(sigmay, res.states)

fig, ax = plt.subplots(tight_layout=True)
ax2 = ax.twinx()
ax.plot(tlist, nc_list, c="tab:blue", label=r"$<n>$")
ax2.plot(tlist, sz_list, c="tab:orange", label=r"$<\sigma_z>$")
ax2.plot(tlist, sx_list, c="tab:green", label=r"$<\sigma_x>$")
ax2.plot(tlist, sy_list, c="tab:red", label=r"$<\sigma_y>$")
ax.set_ylabel("n")
ax.set_xlabel("Time (ns)")
ax.set_ylim(-0.05 * N / 2, 1.05 * N / 2)
...
if False:
    ...
    ax.plot((fftw - wq - chi + wrot2) / np.abs(2 * chi), np.abs(drive_q_fft))
...
fig, ax = plt.subplots(tight_layout=True)
ax2 = ax.twinx()
ax.plot(tlist, np.abs(drive_r), c="tab:blue")
ax2.plot(tlist, np.abs(drive_q), c="tab:orange")
fig.show()
...
if False:
    ...
    for idx in [
            0,  # start
            # len_q,
            # len_r,  # 1st displacement
            len_r + len_q,  # 1st conditional rotation
            len_r + len_q + wait_q,  # wait_q
            len_r + len_q + wait_q + len_q,  # 2nd conditional rotation
            # # len_r + len_q + wait_r,
            # # len_r + len_q + wait_r + len_q,
            # # len_r + len_q + wait_r + len_q + len_r,
            end_q + wait_r // 2,
            end_q + wait_r,
            end_q + 3 * wait_r // 2,
            end_q + 2 * wait_r,
