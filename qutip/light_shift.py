...
amp_r = 4.457591364429735
# amp_r = 0.0
wd_r = (wr - wrot1) - chi
pd_r = -np.pi / 2
drive_r = 0.5 * amp_r * np.exp(+1j * (wd_r * tlist + pd_r))
drive_r[len_r:len_r + wait_r] = 0.0
drive_r[len_r + wait_r + len_r:] = 0.0
# drive_r[len_r:] = 0.0

amp_q = np.pi / T
amp_q = 0.0
wd_q = (wq - wrot2) + chi + 1 * 2 * chi
pd_q = -np.pi / 2
drive_q = 0.5 * amp_q * np.exp(+1j * (wd_q * tlist + pd_q))
if METHOD == "triangle":
    drive_q[:2 * ns] *= triang(2 * ns)
    drive_q[2 * ns:] = 0.0
elif METHOD == "square":
    drive_q[ns:] = 0.0
elif METHOD == "sin2":
    drive_q[:2 * ns] *= np.sin(0.5 * chi * tlist[:2 * ns])**2
    drive_q[2 * ns:] = 0.0
else:
...
for idx in [0, len_r, len_r + wait_r, len_r + wait_r + len_r]:
