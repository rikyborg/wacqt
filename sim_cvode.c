...
static int jac_duffing(realtype t, N_Vector y, N_Vector fy, SUNMatrix J,
    ...
    for (int ii=0; ii<NEQ; ii++) {
        ...
        SM_ELEMENT_D(J, ii, 3) += - 3. * para->duff2 * para->a[ii][3] * P2*P2;
    }
...
static int jac_josephson(realtype t, N_Vector y, N_Vector fy, SUNMatrix J,
    ...
    realtype PHI0 = para->phi0;
    for (int ii=0; ii<NEQ; ii++) {
        SM_ELEMENT_D(J, ii, 3) -= para->a[ii][3];  // remove linear term
        SM_ELEMENT_D(J, ii, 3) += para->a[ii][3] * cos(2.*M_PI*P2/PHI0);  // add cosine term
    }
...
static int jac_josephson_both(realtype t, N_Vector y, N_Vector fy, SUNMatrix J,
                              void *user_data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3) {
    SimPara* para = (SimPara*) user_data;
...
int integrate_cvode(void* user_data,
    ...
    * the initial dependent variable vector y. */
    ...
    if (para->sys_id == ID_LINEAR) {
        ...
        flag = CVDlsSetJacFn(cvode_mem, jac_josephson_both);
    } else {
        printf("*** UNKNOWN FORCE ID: %d ***\n", para->sys_id);
        return(1);
    }
