...
_Ql = _wc / _kappa
AMP = 1.194e-6  # V
save_filename = "fidelity_chi_2e6_kappa_4e5_Nruns_65536_2.npz"
...
def get_envelopes(sol, para, Vg):
    ...
    Vr_spectrum = np.fft.rfft(Vr) / len(Vr)
    ...
    Vr_envelope_spectrum = np.zeros(Npoints, dtype=np.complex128)
    P1_envelope_spectrum = np.zeros(Npoints, dtype=np.complex128)
    V1_envelope_spectrum = np.zeros(Npoints, dtype=np.complex128)

    Vg_envelope_spectrum[karray - para.Nbeats * nc] = Vg_spectrum[karray]
    Vr_envelope_spectrum[karray - para.Nbeats * nc] = Vr_spectrum[karray]
    ...
    Vr_envelope = np.fft.ifft(Vr_envelope_spectrum) * Npoints
...
template_g = Vr_g_envelope.copy()
template_e = Vr_e_envelope.copy()
