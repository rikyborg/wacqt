...
class SimulationParameters(object):
    ...
    def calculate_V0(self, I0, Vg=None, Vn0=None):
        if Vg is None:
            Vg = self.get_drive_V()[:-1]
        if Vn0 is None:
            if self.add_thermal_noise:
                Vn0 = self.noise0_array[:-2]
            else:
                Vn0 = 0.
        return Vg - self.R0 * I0 + Vn0

    def calculate_V2(self, I0, Vn2=None):
        if Vn2 is None:
            if self.add_thermal_noise:
                Vn2 = self.noise2_array[:-2]
            else:
                Vn2 = 0.
